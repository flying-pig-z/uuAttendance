package com.flyingpig.dataobject.constant;

public class RedisConstants {

    public static final Long CACHE_NULL_TTL = 2L;

    public static final String USER_INFO_KEY = "user:info:";

    public static final String LOGIN_USER_KEY = "user:login:";


    public static final Long USER_INFO_TTL = 30L;

    public static final String LEAVE_DETAIL_KEY = "leave-detail:";

    public static final Long LEAVE_DETAIL_TTL = 30L;

    public static final String STUDENT_ATTENDANCE_KEY = "student-attendance:";

    public static final Long STUDENT_ATTENDANCE_TTL = 30L;
}

...
5.在业务层进行事务处理，避免异常导致数据库的操作错误。

6.采用redis对热点数据，如邮箱验证码，用户信息，请假信息等进行缓存，使用缓存空对象避免缓存穿透问题，提高热点数据的存取效率。

7.项目安全性好。使用spring-securiy+jwt对框架对访问接口的用户进行认证，对密码进行哈希加密存储，
并对用户的身份进行精细化鉴权，使用的是RBAC（基于角色的访问控制），确保接口使用的安全性，避免学生调用督导教师接口这类的异常发生。

8.使用websocket长连接，每隔几秒就向数据库查询数据有无更新，如果有更新就发送给前端。使得学生考勤签到界面的信息实时更新，解决轮询消耗大量带宽的问题。

9.在学生定位签到时候使用RabbitMQ将更新数据库签到状态的操作异步进行
...
3.签到页面属于高并发，如果并发量大的时候需要利用Redis对签到信息进行存储来优化签到接口。
